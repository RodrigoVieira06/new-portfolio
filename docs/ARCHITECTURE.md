...
Este repositório contém o portfólio público de Rodrigo Vieira Lima. A prioridade é apresentar seu perfil, trabalhos e trajetória profissional confirmada de modo objetivo, acessível, responsivo e fácil de manter.
...
- `src/data/portfolio.ts` é a fonte única para experiências, progressão de cargos, projetos, formação e skills. `docs/personal/Profile.pdf` é referência documental complementar; alterar os dados tipados antes de alterar a visualização.
...
- O `Header` usa o novo símbolo geométrico transparente da marca. A prop `theme` seleciona `src/assets/logo/logo-rv.png` no tema claro e `src/assets/logo/logo-rv-monochrome.png` no tema escuro, preservando contraste sem alterar dimensões.
- Os assets locais são organizados por finalidade. `src/assets/brand/` preserva os arquivos-fonte da identidade, `src/assets/logo/` concentra as variantes consumidas pela interface e `src/assets/personal/` as fotos pessoais; demais imagens devem entrar em subpastas semânticas ao serem incluídas, sem assets na raiz de `src/assets/`.
...
- A paleta deriva da nova marca: verde vivo, verde secundário e cinzas, combinados com superfícies claras e escuras. `src/styles/globals.scss` mantém os tons originais em preenchimentos e variantes mais contrastantes para texto e foco.
...
- `Experience` associa as imagens de `src/assets/experience/` às empresas e pode exibir `previousRoles` para representar progressões confirmadas dentro da mesma organização. `Portfolio` usa `src/assets/projects/` e mantém LIPCAT fora do histórico empresarial. `Education` usa `src/assets/education/` para FAETERJ-Rio e IFRJ.
...
- Jest e Testing Library cobrem o conteúdo estrutural, posicionamento profissional, alternância de tema, navegação do carrossel, progressão de carreira, formação e validação do formulário.
